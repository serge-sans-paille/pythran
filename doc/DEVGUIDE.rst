...
NT2 is required to compile the C++ code generated by Pythran. It is automatically installed along Pythran.
If you don't have NT2 installed in a system directory *and* you try to run Pythran directly from the source directory,
the NT2 includes won't be found.

A trick to ease the development and to enable running directly from the source is to
install Pythran once and to add ``-I<install_prefix>/lib/<python-version>/site-packages/pythran``
to the ``cppflags`` in order to point to the NT2 headers.
