...
evaluation. This is always legal in the contect of Numpy because none of the
...
A naive way to parallelize the parameteric loop nest from
...
parallel regions are activated, each parallel region spaws as many thread as
possible, which leads to oversubscription and generally bad performances. When
...
costly overhead. The \texttt{collapse} clause is not compatible with the
parametric loop version.

A first approach would be to use recursieve tasks and manual loop chunking. \todo{pierrick? c'est bête comme idée ?}
