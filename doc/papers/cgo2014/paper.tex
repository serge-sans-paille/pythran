...
global variables to determine if a function has side effects on its argument or
...
As introduce in \S~\ref{sec:stackoverflow}, we use the numpy-benchmark test suite
to evaluate these optimisation. It is use with Pythran (commit: 488b7ba)
with optimization enabled/disabled. Finally, we compare result with Pypy (???),
Parakeet (commit: 4e55810), Numba (commit: b8a79c5) and Python 2.7.

Benchmarking is done one two architecture with different backend compilers.
...
up to SSE4. We will refer to this configuration as \textbf{AMD48}.
...
To compare optimisation result, we run the numpy-benchmarks test suite multiple
time on the rosen.py benchmark. Results on Figure~\ref{rosenamd48.png} and \ref{rosenintel8.png} show difference
one speed which could be explain by AVX support on the intel8 architecture while amd48
support up to SSE4 only.
% Haaaaa, I don't understand why OMP is not use...
% tODO : I have to redo mesures without forward substitution to show difference.

\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{rosen_amd48.png}
\caption{Rosen on AMD48}
\label{fig:rosenamd48}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{rosen_intel8.png}
\caption{Rosen on Intel8}
...
use of technique described in this paper. As we can see, both SIMD instruction
...
Also, we can notice that gcc as a backend compiler have more optimization opportunity.

\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{arc_distance_amd48.png}
\caption{Arc distance on AMD48}
\label{fig:arc_distance}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{arc_distance_intel8.png}
\caption{Arc distance on Intel8}
\label{fig:arc_distance_intel8}
\end{figure}

At the opposite, Figure~\ref{fig:diffusion} show that clang is better when generated
code make a more intensive use of templates (heavily use with gsliced\_ndarray).
gsliced\_ndarray is a non aligned view on an array in this case so SIMD optimization
can't be applied on the second dimension even if loop fusion and temporary
elimination is performed.  We can notice that Parakeet perform really well on
extensive gslice use.
%TODO IL faut refaire les mesures en changeant la limite OpenMP. Parce qu'on a
%pas du tout de parallelisme la. On note par contre l"overhead libgomp...

\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{diffusion_amd48.png}
\caption{Diffusion on AMD48}
\label{fig:diffusion}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{diffusion_intel8.png}
\caption{Diffusion on Intel8}
...
\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{l2norm_amd48.png}
\caption{L2norm on AMD48}
\label{fig:l2norme}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[scale=0.4]{l2norm_intel8.png}
\caption{L2norm on Intel8}
