...
As introduced in \S~\ref{sec:stackoverflow}, we use the numpy-benchmark test suite
to evaluate the optimizations described in\S~\ref{sec:pythran}. It uses Pythran (commit: 488b7ba)
with optimization enabled/disabled. The results are compared with Pypy \todo{add}(???),
Parakeet (commit: 4e55810), Numba (commit: b8a79c5) and Python 2.7.

Benchmarking is done on two architectures with different backend compilers. The
\texttt{timeit} module is used to perform the measurments, taking the average
execution time over several runs.
...
%FIXME: je me demande si on devrait pas utiliser juste cette archi. Qu'est ce que l'autre apporte ? On sous utilise complètement les 48 coeurs...
...
% SG: may be the number of iteration is not enough? what array size did you use? (it should be specified in the text secion, BTW
...
%FIXME: mehdi propose de n'utiliser qu'un backend pour centrer le propos sur Pythran/Numpy.
% ça allègera aussi les graphes...
