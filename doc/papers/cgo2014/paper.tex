...
allocation/disallocation costly operation. In the rosen expression from
...
better performance to the price of API bloat. With a smamm set of functions, a loop is used for
...
At the opposite, if all computation were merged into a single loop using
Temporaries Elimination and Operator fusion as described above, parallelization
would be applicable as barrier between binary operation are removed and only
one load and one store will be use for multiple operations.
...
\texttt{reduce}s. As it applies optimizations like operator fusion, it
...
A first approach would be to use recursive tasks and manual loop chunking. \todo{pierrick? c'est bête comme idée ?}
