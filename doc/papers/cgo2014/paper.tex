...
compiler and provided a benchmarks testsuite for performance measures with
...
Newcommers to Python with a prior C or Fortran experience are likely to make 
use of raw loops similar to Listing~\ref{lst:l2norm-loopy}, but as their 
knowledge of the Numpy API and of the good programming practice grows, they get to write code similar
to Listing~\ref{lst:l2norm}. As a direct consequence, 
\todo{Serge: don't you think we should also says that high-level constructs 
enable better optimizations for the compiler?}
a compiler should focus on optimizing high-level constructs rather than raw 
...
allocation/disallocation costly operation. In the \textit{rosen} function from
...
memory load store would have dominated. This is even more important as
Numpy typically operates on double precision floats. Parallelization also 
suffers from the lack of operator fusion: a synchronization fence is needed
between each temporary computation. 
Hence the loop computation intensity would be very low compared to
...
of programming can be very efficient with Numpy when compared to regular Python
code. However the main limitation is that the Numpy library approach comes with
numerous temporary arrays implicitly allocated.
...
of parallel primitives including \texttt{map}, \texttt{scan}, and
\texttt{reduce}. As it applies optimizations such as operator fusion, it
encourages programmer to express their code using the higher-level construct
...
identify hot-spots that are JIT-compiled to native code. As it requires to write
functional code, it needs code modifications of \textit{Pythonic} code
...
heading toward STM for parallelism support. As it supports most of the python
construct, python code can be directly use with performance gain but not all
...
\subsubsection{Parallelisation of Expression templates}
...
costly overhead for scheduling computation and datasharing. An easy way to solve
...
regions to do so in order to prevent the unnecessary overhead. Instead the loop
statement is duplicated and guarded by the appropriate check, where only one
loop holds the parallel annotations.
% usuellement appelé : manual if
...
A first exploration of vectorization of Python constructs is done in~\cite{wpmvp2014}.
