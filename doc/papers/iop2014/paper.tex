...
\begin{abstract}
    ...
    % It is more a classical "use" of the language
    ...
    the original code. 
    % It may be a little too ... untactful?
...
optimizations, such as vectorization, %FIXME too long
while they are available at higher level,
i.e. at the Python level. Taking into account the specificities of the Python
language can unlock many new transformations. %FIXME sentence without subject
...
classes are either used to generate regular C++ % FIXME (not clear what do you mean?) PB : Better?
...
\begin{itemize}
    ...
    \item the closure of nested functions is statically computed to turn nested
        ...
        % FIXME (this sentence makes my head hurt :s maybe cut into smaller sentences)
    ...
    \item \texttt{try \dots finally} constructs are turned into nested \texttt{try \dots except} blocks;
        % FIXME (Is the difference between construct and block obvious for intended readers?)
...
an integer. % FIXME (not sure if clear enough) 
...
% FIXME (tooooooooooooooooooooooo long) 
...
Although far more verbose % FIXME  (don't think you can use verbose this way)
...
module. % FIXME (not only the functions called by top-level functs????)
...
\texttt{ImportedIds} that computes the set of % FIXME (too long)
...
vectorized version.
% You complain about 4x because you only have 3.98x ???
...
def pi_estimate(darts):
    ...
    return pi
\end{lstlisting}
% why #? I thought string was enough?
...
% FIXME (Ca semble tomber a plat, pas de resultats, pas de conclusion...)
...
benchmark. PyPy generates a code almost as fast % FIXME (dunno if it's efficient) 
...
The speed ups are not amazing % FIXME (not formal enough, same for previous usage) 
...
% Showing parallelization AND np.array at the same time confuse reader as we
% don't know how much each optimisation contribute in the speed up
...
\begin{table}
    ...
    \hline
\end{tabular}
% FIXME (You changed the order of the compilers, even though pythran is still first why?)
...
% In fact there are small benefits for compiling this code using only pythran without AVX and openMP.
...
the generated code runs around 176ms instead of 36ms. %FIXME speedup in x?
