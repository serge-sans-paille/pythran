...
\author{
    \IEEEauthorblockN{Serge Guelton}
    \IEEEauthorblockA{
        {\'E}cole Normale Sup{\'e}rieure, Paris, France\\
        Email: \texttt{serge.guelton@telecom-bretagne.eu}\\
    }
    \and
    \IEEEauthorblockN{Pierrick Brunet}
    \IEEEauthorblockA{
        T{\'e}l{\'e}com Bretagne, Plouzan{\'e}, France\\
        Email: \texttt{pierrick.brunet@telecom-bretagne.eu}\\
    }
    \and
    \IEEEauthorblockN{Mehdi Amini}
    \IEEEauthorblockA{
        Silkan, USA\\
        Email: \texttt{mehdi.amini@silkan.com}
    }
}
...
applications. However, the prohibitive performance overhead of the language
implied by its interpreted and highly dynamic nature should prevent its usage
for critical code sections, where native code is generally used. The SciPy
modules overcomes this issue through low-level routines written in C or Fortran
and encapsulated in Python \emph{native} modules.  Moreover, its core data
structure, the multi-dimensional array~\cite{numpyarray2011}, has been designed
so that the underlying data are available to both native modules and the Python
interpreter without conversion cost.
...
widespread in the Python landscape. Section~\ref{sec:python-parallel} gives an
overview of existing approaches to compile Python functions or modules and shows
a critical lack of parallelism support, and emphasises on the need for a
backward-compatible approach.  Section~\ref{sec:python-static} presents the
Pythran~\cite{pythran2013} compiler, a tool that turns Python modules into C++
meta-programs, and the underlying runtime. Section~\ref{sec:python-openmp}
...
The Pythran compiler is benchmarked on several scientific kernels and compared
to Cython in Section~\ref{sec:validation}.
...
construct, numba falls back to Python C-API calls, while parakeet just raises an
exception.
...
instruction to apply an annotation to a whole block. Pythran uses comment to
...
clauses. We propose an algorithm to automatically limit the scope of a variable
declaration so that it is can be automatically marked as private. The whole idea
is to compute the minimal nesting depth of a variable usage. To do so, a
\emph{nesting depth} is first associated to each expression. It lexically
corresponds to the indentation level of its instruction, to the notable
exception of the iterator declaration in a for loop, that holds a
...
Thanks to this automatic scoping computation, it is possible to write shorted
OpenMP directive in a similar manner to C, as illustrated in
...
well known problem with OpenMP: nested parallel regions. OpenMP provides several
...
summarizes the results of the experimentation made on a laptop with 4 non
hyperthreaded cores using \texttt{-O2} compiler flag. It not only emphasises the
...
\texttt{timeit} module on a machine using 8 AMD Opteron 6176 SE.  All results
