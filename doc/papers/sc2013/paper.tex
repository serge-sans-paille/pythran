...
\begin{abstract}
    ...
    High Performance Computing users traditionally rely on low-level,
    compiled language such as C or
    FORTRAN to perform compute-intensive tasks. As a consequence, it is a
    common situation to have High Performance Computing application written in a high-level language
    such as Python, calling native routines for compute-intensive tasks. To
...
compiled code, and to overcome the GIL limitations, Python developers has to write
...
To be completely backward compatible, it has to be possible to run the input code
...
with the Python subset supported by the Pythran compiler to make possible to
...
library ---\texttt{pythonic++}--- to allow a one-to-one mapping between Python and
C++ constructs. The purpose of this section is not to dig into the internals of
the compiler, but rather to focus on the impact of the parallelism layer,
...
Pythran handles the problem by refusing recursive types, which makes possible
...
apply to structured blocks in C/C++ and delimited by comments in Fortran.
A few directives (e.g.\ \texttt{threadprivate}) are not
...
chains, makes possible to build a list of nesting depths, one per variable
...
of \{DEF, USE\}, nesting depths of \{2, 2\} and a scope of 2, which means it can be
...
OpenMP provides a small runtime library that, for instance, makes possible to
...
In addition to the experimental validation done using the OpenMP validation suite
...
Figure~\ref{fig:pb-pairwise} shows the execution time of the computation of a distance matrix between two random vectors, using a parallel nested loop. On that example, Pythran slightly outperforms Cython, parakeet and Numba. Taking into account the OpenMP annotation that flags the outermost loop as parallel yields an $\times 4.27$ speedup to Pythran.
...
Figure~\ref{fig:pb-rosen} shows the execution time of the computation of the third derivative of the Rosenbrock function for a random array input, using Numpy's vector operations. There is no explicit parallelism in this function but the vector operation is implictly parallel. On this example, Cython is on par with Pythran and slightly faster than Parakeet, but turning OpenMP on makes the Pythran version run $\times 1.78$ faster than sequential Pythran.
...
Figure~\ref{fig:pb-rosen} shows the execution time of the computation of one step of the GrowCut algorithm used in image processing. On this example, Parakeet performs better than Cython and Pytrhan and way better than Numba, but paralleizing the outermost loop with OpenMP gives a $\times 2.65$ speedup to Pythran that helps it outperform Parakeet.
...
Taking advantage of this parallelism appears to be the key to reach an additional performance level.
...
could be exposed at the python level. There are also several implicit vectorization
