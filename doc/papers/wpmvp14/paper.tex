...
measurements are made using the \texttt{timeit} module.
...
numbers. Table~\ref{tbl:map-timings} summarizes the result. The execution time
...
\begin{table}
    ...
    \begin{tabular}{|l|r|r|r|}
        \hline
                                               & \textbf{CPython}   & \textbf{Pythran}   & \textbf{Pythran+AVX} \\
        \hline
        \texttt{map(abs, r)}                    &   1530ms  & 1.72ms    & 0.587ms   \\
        \hline
        \texttt{map(f, r)}    &   141ms   & 1.72ms    & 0.587ms  \\
        \hline

    \end{tabular}
    \caption{Timings of various \texttt{map} calls using CPython, Pythran, and Pythran with vectorization.}
    \label{tbl:map-timings}
...
Table~\ref{tbl:sum-timings} summarizes the result. Again, CPython is extremely
...
Table~\ref{tbl:numpy-timings} summarizes the results. Because all computations
...
\begin{table}
    ...
    \begin{tabular}{|l|r|r|r|}
        \hline
                                               & \textbf{CPython}   & \textbf{Pythran}   & \textbf{Pythran+AVX} \\
        \hline
        \texttt{1. / a + a}                    &   7.48ms  & 6.19ms    & 4.05ms   \\
        \hline
        \emph{rosen}                           &   13.9ms   & 2.98ms   & 1.15ms  \\
        \hline
        \emph{arc distance}                    &   178ms   & 146ms    & 53.8ms  \\
        \hline

    \end{tabular}
    \caption{Timings of various \texttt{numpy} expressions using CPython, Pythran, and Pythran with vectorization.}
    \label{tbl:numpy-timings}

\end{table}

\subsection{Vectorization of an Euler Problem}

This experiment considers the code from Listing~\ref{lst:euler06-py}.
...
\begin{table}
    ...
    \begin{tabular}{|l|r|r|r|}
        \hline
                                               & \textbf{CPython}   & \textbf{Pythran}   & \textbf{Pythran+AVX} \\
        \hline
        \emph{euler06}                    &   12ms   & 0.34ms    & 0.30ms  \\
        \hline
