...
\begin{abstract}
    ...
    a consequence, programs run using Python typically run an order of magnitude
...
object are unboxed when transfered from Python to C, where efficient
...
load / stores and only two additions per loop iteration and vectorization is
...
Listing~\ref{lst:numpy-code} and avoid the creation of any temporary in some
case detected with static analysis of the source code.
