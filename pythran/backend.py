...
from pythran.analysis import DeclaredGlobals, OrderedGlobalDeclarations
...
class Cxx(Backend):
    ...
    def __init__(self):
        ...
        super(Cxx, self).__init__(Dependencies, GlobalDeclarations, Types,
                                  ...
                                  DeclaredGlobals, OrderedGlobalDeclarations)
    ...
    def visit_Module(self, node):
        ...
        #Also, when possible, we need to have the same order as in the types
        # analysis, aka the same order as OrderedGlobalDeclarations.
        #
        #This is because of interprocedural argument type deduction (check
        # tests/cases/ramsurf.py, the epade function and the types/ptypes
        # generated. Try to run it without the variable below a few times, and
        # an error of undefined 'epade_lambda5' should pop up)
        original_order = self.ordered_global_declarations
        original_order += list(set(order.nodes()) - set(original_order))

        #If a depends on b, we want b first, so reverse the graph and sort it
        order.reverse(False)
        declarations = [node_to_declaration[node]
                        for node in nx.topological_sort(order, original_order)]
