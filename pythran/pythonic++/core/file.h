...
namespace pythonic {
    ...
    namespace core {
        ...
        struct _file {
            ...
            size_t _buffer_size;
            _file() :
                    f(nullptr), _buffer(nullptr), _buffer_size(0)
            {
            }
            _file(core::string const& filename, core::string const& strmode = "r") :
                    f(fopen(filename.c_str(), strmode.c_str())), _buffer(nullptr), _buffer_size(0)
            {
            }
            FILE* operator*() const
            {
        ...
        public:
            ...
            file(core::string const& filename, core::string const& strmode = "r") :
                    data(), mode(strmode), name(filename), newlines('\n')
            {
            ...
            iterator end()
            {
                return iterator(*this, core::npos());
            }
            ...
            core::string read(int size = -1)
            {
                ...
                if (size == 0 or (feof(**data) && mode.find_first_of("ra") == std::string::npos))
                    return core::string();
                int curr_pos = tell();
                seek(0, SEEK_END);
                ...
                // FIXME This part needs a new implementation of core::string(char*) to avoid unnecessary copy.
            ...
            core::string readline(ssize_t size = std::numeric_limits<ssize_t>::max())
            {
                ...
                // FIXME This part needs a new implementation of core::string(char*, size_t) to avoid unnecessary copy.
                if (real_size > 0)
                    return core::string(data->_buffer, std::min(real_size, size));
                else
                    return core::string();
            }
        ...
        file_iterator::file_iterator(core::file & ref) :
                f(ref), curr(ref.readline()), position(ref.tell())
        {
        }
        ;
        file_iterator::file_iterator(core::file & ref, core::npos) :
