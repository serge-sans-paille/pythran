...
namespace pythonic {
    ...
    namespace __builtin__ {
        ...
        /* chr */
        template<class T>
        core::string chr(T const &v)
        {
            return core::string((char) v);
        }
        ...
        core::dict<
                ...
                typename std::tuple_element<1, typename std::remove_reference<Iterable>::type::iterator::value_type>::type> dict(Iterable&& iterable)
        {
            core::dict<
                    typename std::tuple_element<0, typename std::remove_reference<Iterable>::type::iterator::value_type>::type,
        ...
        struct _enumerate: enumerate_iterator<typename Iterable::iterator> {
            ...
            _enumerate(Iterable seq, int first) :
                    iterator(seq.begin(), first), seq(seq), end_iter(seq.end(),-1)
            {
            }
            iterator & begin()
            {
        ...
        template<class Iterable>
        _enumerate<typename std::remove_cv<typename std::remove_reference<Iterable>::type>::type> enumerate(
                Iterable && seq, long first = 0L)
        {
        ...
        core::list<typename Iterable::iterator::value_type> filter(F const& f, Iterable const& iterable)
        ...
        template<class ... Types>
        core::list<typename std::tuple_element<0, std::tuple<Types...>>::type> list(std::tuple<Types...> const & other)
        {
...
#define PYTHONIC_EXCEPTION(name) \
        ...
        {
            core::list<std::tuple<typename std::remove_reference<List0>::type::iterator::value_type,
        ...
        /* pow2 */
        template<class T>
        auto pow2(T const& e) -> decltype(e*e)
        {
            return e * e;
        }
        ...
        struct xrange {
            ...
            {
                if (_step > 0)
                    _last = _begin + std::max(0L, _step * ((_end - _begin + _step - 1) / _step));
                else
                    _last = _begin + std::min(0L, _step * ((_end - _begin + _step + 1) / _step));
            }
            xrange()
            {
            }
            ...
            {
                return reverse_iterator(_last - _step, -_step);
            }
            reverse_iterator rend() const
            {
                return reverse_iterator(_begin - _step, -_step);
            }
        };
            ...
            -> decltype( std::accumulate(s.begin(), s.end(), static_cast<decltype(op(init,std::declval<typename Iterable::iterator::value_type>()))>(init), op) )
        {
            return std::accumulate(s.begin(),
                                   s.end(),
                                   static_cast<decltype(op(init,std::declval<typename Iterable::iterator::value_type>()))>(init),
                                   op);
        }
        ...
        auto tuple_sum(Tuple const& t,
                ::pythonic::int_<I>) -> typename std::remove_reference<decltype(std::get<I>(t))>::type
        {
        ...
        core::list<std::tuple<typename Iterator0::value_type, typename Iterators::value_type...> > _zip(size_t n,
                Iterator0 first, Iterators ... iters)
        {
        ...
        core::list<
                std::tuple<typename std::remove_reference<List0>::type::value_type,
                        typename std::remove_reference<Lists>::type::value_type...> > zip(List0 && s0,
                Lists &&... lists)
        {
