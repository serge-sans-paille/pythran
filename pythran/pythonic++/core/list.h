...
namespace pythonic {
    ...
    namespace core {
        ...
        public:
            ...
            list_view(list_view<T> const & s) :
                    data(s.data), slicing(s.slicing)
            {
            }
            list_view(list<T> & other, slice const & s) :
            ...
            {
                return (*data)[slicing.lower + i * slicing.step];
            }
            T & operator[](long i)
            {
                return (*data)[slicing.lower + i * slicing.step];
        ...
        public:
            ...
            list(size_type sz) :
                    data(impl::shared_ref<container_type>::make_ref(sz))
            {
            }
            list(T const& value, single_value) :
                    data(impl::shared_ref<container_type>::make_ref(1))
            {
                (*data)[0] = value;
            }
            list(std::initializer_list<T> l) :
                    data(impl::shared_ref<container_type>::make_ref(std::move(l)))
            {
            }
            list(list<T> && other) :
            ...
            list(list<F> const & other) :
                    data(impl::shared_ref<container_type>::make_ref(other.size()))
            {
                std::copy(other.begin(), other.end(), begin());
            }
            template<class ... Types>
            list(std::tuple<Types...> const& t) :
                    data(impl::shared_ref<container_type>::make_ref(sizeof...(Types)))
            {
                tuple_dump(t, *this, int_<sizeof...(Types)-1>());
            }
            list(list_view<T> const & other) :
                    data( impl::shared_ref<container_type>::make_ref(other.begin(), other.end()))
            {
        ...
        struct empty_list {
            ...
            template<class T>
            list_view<T> operator+(list_view<T> const & s) const
            {
                return s;
            }
...
/* overload std::get
 * FIXME: overload in namespace std is not allowed by the standard. Only template specialization.
 * I think these could be changed in specialization.
 *
 * 7.6.2.4.1 [namespace.std]
 * The behavior of a C++ program is undefined if it adds declarations or definitions to namespace
 * std or to a namespace within namespace std unless otherwise specified. A program may add a
 * template specialization for any standard library template to namespace std only if the
 * declaration depends on a user-defined type and the specialization meets the standard library
 * requirements for the original template and is not explicitly prohibited.
 *
 */
namespace std {
    template<size_t I, class T>
    typename pythonic::core::list<T>::reference get(pythonic::core::list<T>& t)
    {
        return t[I];
    }
    template<size_t I, class T>
    typename pythonic::core::list<T>::const_reference get(pythonic::core::list<T> const & t)
    {
        return t[I];
    }
