...
namespace pythonic {
    ...
    namespace core {
        ...
        struct broadcast {
            ...
            broadcast(T v) :
                    __value(v), _value(vectorized<T>::broadcast(v))
            {
            }
#ifdef USE_BOOST_SIMD
            typename vectorized<T>::type load(long ) const
            {
                return _value;
            }
#endif
        ...
        struct numpy_uexpr {
            ...
            numpy_uexpr(Arg0 const& arg0) :
                    arg0(arg0), shape(arg0.shape)
            {
            }
#ifdef USE_BOOST_SIMD
        ...
        typename std::enable_if<U::value != 0, core::array<long, N>>::type const& select_shape(U const& u, V const&,
                int_<N>)
        {
            return u.shape;
        }
        template<class U, class V, size_t N>
        typename std::enable_if<U::value == 0 and V::value != 0, core::array<long, N>>::type const& select_shape(
                U const&, V const&v, int_<N>)
        {
            return v.shape;
        }
        ...
        struct numpy_expr {
            ...
            static constexpr size_t value =
                    ( (std::remove_reference<Arg0>::type::value) > (std::remove_reference<Arg1>::type::value) ) ?
                            std::remove_reference<Arg0>::type::value :
                            std::remove_reference<Arg1>::type::value;
            typedef typename type_helper<ndarray<decltype(Op()(arg0.at(std::declval<long>()),
                                                               arg1.at(std::declval<long>()))), value>>::type value_type;
            core::array<long, value> shape;
            numpy_expr()
            {
            }

            numpy_expr(Arg0 arg0, Arg1 arg1) :
                    arg0(arg0), arg1(arg1), shape(select_shape(arg0, arg1, int_<value>()))
            {
            }
#ifdef USE_BOOST_SIMD
            ...
            long size() const
            {
                return std::max(arg0.size(), arg1.size());
            }
        };
        ...
        struct nditerator;
        template<class T, size_t N>
        struct nditerator<ndarray<T, N>> : std::iterator<std::random_access_iterator_tag,
                typename std::remove_reference<decltype(std::declval<ndarray<T,N>>()[0])>::type>
        {
            ndarray<T, N> data;
            typename std::remove_reference<decltype(std::declval<ndarray<T,N>>()[0])>::type iter;
            long step;
            nditerator(ndarray<T, N> const& data, long index) :
                    data(data), iter(data[index]), step(type_helper<ndarray<T, N>>::step(data))
            {
            }
            decltype(iter) const& operator*() const
            {
                return iter;
            }
            ...
            nditerator<ndarray<T, N>>& operator++()
            {
                iter.buffer += step;
                return *this;
            }
            ...
            nditerator<ndarray<T, N>> operator+(long i) const
            {
                nditerator<ndarray<T, N>> other(*this);
                other.iter.buffer += i * step;
                return other;
            }
            nditerator<ndarray<T, N>> operator-(long i) const
            {
            ...
            nditerator<ndarray<T, N>>& operator+=(long i)
            {
                iter.buffer += i * step;
                return *this;
            }
            ...
            long operator-(nditerator<ndarray<T, N>> const& other) const
            {
                return (iter.buffer - other.iter.buffer) / step;
            }
        ...
        struct nditerator<ndarray<T, 1>> : std::iterator<std::random_access_iterator_tag, T>
        {
            ndarray<T, 1> data;
            long index;
            nditerator(ndarray<T, 1> const& data, long index) :
                    data(data), index(index)
            {
            }
            auto operator*() const -> decltype(data[index])
            {
                return data[index];
            }
            auto operator*() -> decltype(data[index])
            {
                return data[index];
            }
            nditerator<ndarray<T, 1>>& operator++()
            {
                ++index;
                return *this;
            }
            ...
            nditerator<ndarray<T, 1>> operator+(long i) const
            {
                return nditerator<ndarray<T, 1>>(data, index + i);
            }
            nditerator<ndarray<T, 1>> operator-(long i) const
            {
                return nditerator<ndarray<T, 1>>(data, index - i);
            }
            nditerator<ndarray<T, 1>>& operator+=(long i)
            {
                index += i;
                return *this;
            }
            ...
            long operator-(nditerator<ndarray<T, 1>> const& other) const
            {
        ...
        struct indexed_ndarray: ndarray<T, N>
        {
            ...
            indexed_ndarray(ndarray<T, N> const& t) :
                    ndarray<T, N>(t)
            {
            }
            indexed_ndarray(ndarray<T, N + 1> const& t, long i)
            {
                ...
                ndarray<T, N>::buffer = t.buffer
                        + i * std::accumulate(t.shape.begin() + 1, t.shape.end(), 1L, std::multiplies<long>());
            ...
            template<class E>
            typename std::enable_if<is_array<typename std::remove_reference<E>::type>::value, indexed_ndarray<T, N>&>::type operator=(E&& e)
            {
                if (ndarray<T, N>::data_size) {
                    for (long i = 0, n = e.size(); i < n; ++i)
                        ndarray<T, N>::buffer[i] = e.at(i);
            ...
            template<class E>
            typename std::enable_if<not is_array<typename std::remove_reference<E>::type>::value, indexed_ndarray<T, N>&>::type operator=(E&& e)
            {
                ndarray<T, N>::operator=(std::forward<E>(e));
                return *this;
            }
        ...
        struct type_helper<ndarray<T, N>>
        {
            ...
            {
                return std::accumulate(self.shape.begin() + 1, self.shape.end(), 1L, std::multiplies<long>());
            }
        };
         ...
         */
        slice const& as_slice(slice const& s)
        {
            return s;
        }
        ...
        struct gsliced_ndarray {
            ...
            gsliced_ndarray(T const& data, core::array<slice, M> const& s, core::array<bool, M> const& mask) :
                    gslice(), gshape(), shape(), data(data), to_index(impl::shared_ref<raw_array<long>>::make_ref())
            {
            ...
            template<size_t L>
            void fill_index(int offset, int indice, int_<L>,
                            core::array<long, M> const& jump_array,
                            core::array<long, M> const& jump_extslice)
            {
            ...
            void fill_index(int offset, int indice, int_<1>,
                    core::array<long, M> const&,
                    core::array<long, M> const&)
            {
            ...
            template<size_t V>
            auto get(long i, int_<V>, int_<1>) const -> indexed_ndarray<typename nested_container_value_type<T>::type, value - 1>
            {
        ...
        struct sliced_ndarray: normalized_slice {
            ...
            sliced_ndarray(T const& data, slice const& s) :
                    normalized_slice(s.normalize(data.shape[0])), shape(data.shape), data(data)
            {
            ...
            long size() const
            {
                return (data.size() / data.shape[0]) * shape[0];
            }
            reference operator[](long i)
            {
                return data[jump * lower + i * step];
            }
            ...
            template<class E>
            typename std::enable_if<(value > 1) and core::is_array<E>::value, sliced_ndarray<T>&>::type operator=(
                    E const& v)
            {
...
#pragma omp parallel for if(normalized_slice::size() > 10000)
        ...
        struct ndarray {
            ...
            /* constructors */
            ndarray() :
                    data_size(0), mem(), buffer(nullptr), shape()
            {
            }

            /* copy */
            ndarray(ndarray<T, N> const& other) :
                    data_size(other.data_size), mem(other.mem), buffer(other.buffer), shape(other.shape)
            {
            }

            /* move */
            ndarray(ndarray<T, N> && other) :
                    data_size(other.data_size), mem(std::move(other.mem)), buffer(std::move(other.buffer)), shape(std::move(other.shape))
            {
            }
            ...
            template<class Iterable,
                    class = typename std::enable_if<
                                        // prevent destruction of copy constructor
                                        !std::is_same<typename std::remove_cv<typename std::remove_reference<Iterable>::type>::type,
                                                      ndarray<T, N>>::value
                                        and !is_numpy_expr<typename std::remove_cv<typename std::remove_reference<Iterable>::type>::type>::value
                                        and is_iterable<typename std::remove_cv<typename std::remove_reference<Iterable>::type>::type>::value, void>::type>
...
#pragma omp parallel for if(n>1000)
            ...
            /* through iterators */
            nditerator< ndarray<T,N> > begin()
            {
                return nditerator<ndarray<T,N> >(*this, 0);
            }
            nditerator< ndarray<T,N> > begin() const
            {
                return nditerator<ndarray<T,N> >(*this, 0);
            }
            nditerator< ndarray<T,N> > end()
            {
                return nditerator<ndarray<T,N> >(*this, data_size);
            }
            nditerator< ndarray<T,N> > end() const
            {
                return nditerator<ndarray<T,N> >(*this, data_size);
            }
        ...
        struct numpy_expr_to_ndarray<numpy_uexpr<Op, Arg>> {
            typedef typename std::remove_cv<
                    typename std::remove_reference<decltype(std::declval<numpy_uexpr<Op, Arg>>().at(0))>::type>::type T;
            static const size_t N =
                    std::tuple_size<typename std::remove_cv<typename std::remove_reference<decltype(std::declval<numpy_uexpr<Op, Arg>>().shape)>::type>::type>::value;
            typedef core::ndarray<T, N> type;
        };
        ...
        struct numpy_expr_to_ndarray<gsliced_ndarray<E, M, L>> {
            typedef typename std::remove_cv<
                    typename std::remove_reference<decltype(std::declval<gsliced_ndarray<E,M,L>>().at(0))>::type>::type T;
