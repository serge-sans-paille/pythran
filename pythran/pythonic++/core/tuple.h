...
namespace pythonic {
    ...
    namespace core {
        template<class T, size_t N>
            ...
            template<class T, size_t N>
                struct ltuple : public std::array<T,N> {
                    ltuple(std::initializer_list<T> l) : std::array<T,N>() {
                        std::copy(l.begin(), l.end(), this->begin());
                    }
                    ltuple() : std::array<T,N>() {}
                    ltuple(ltuple<T,N> const& l) : std::array<T,N>(l) {}
                    ltuple(ltuple<T,N>&& l) : std::array<T,N>(std::move(l)) {}
                    template<class U>
                        ltuple(ltuple<U,N> const& l) : std::array<T,N>() {
                            std::copy(l.begin(), l.end(), this->begin());
                        }
                    template<class... Types>
                    ...
                    template<class T1, size_t N1>
                        bool operator==(pythonic::core::ltuple<T1,N1> const& other) const {
                            return N == N1 and std::equal(this->begin(), this->end(), other.begin());
                        }
                    core::ltuple<T,N>& operator=(core::ltuple<T,N> const& t){
                    ...
                    /* ltuple */
                    friend std::ostream& operator<<(std::ostream& os, core::ltuple<T,N> const & v) {
                        os << '(';
                        auto iter = v.begin();
                        if(iter != v.end()) {
                            while(iter+1 != v.end())
            ...
            template<class... Types>
                struct _make_tuple<true, Types...> {
                    core::ltuple<typename are_same<Types...>::type, sizeof...(Types)> operator()(Types... types) {
                        typedef typename are_same<Types...>::type T;
                        return core::ltuple<T, sizeof...(Types)>({types...});
                    }
                };
            ...
            template<class T, size_t N, class... Types>
                auto operator+(std::tuple<Types...> const& t, core::ltuple<T,N> const& lt)
