...
namespace pythonic
{
  ...
  namespace __builtin__
  {
    ...
    template <class Iterable>
    typename _enumerate<Iterable>::iterator const &
    _enumerate<Iterable>::begin() const
    {
      return *this;
    }
    ...
    template <class Iterable>
    _enumerate<typename std::remove_cv<
        typename std::remove_reference<Iterable>::type>::type>
    enumerate(Iterable &&seq, long first)
    {
      return _enumerate<typename std::remove_cv<
          typename std::remove_reference<Iterable>::type>::type>(
          std::forward<Iterable>(seq), first);
    }
