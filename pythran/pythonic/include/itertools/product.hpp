...
namespace pythonic
{
  ...
  namespace itertools
  {
    ...
    struct _product : product_iterator<decltype(std::make_tuple((
                                           *std::declval<Iters>().begin())...)),
                                       Iters...> {
      using value_type =
          decltype(std::make_tuple((*std::declval<Iters>().begin())...));
    ...
    _product<typename std::remove_cv<
