...
typename std::enable_if<pythonic::types::is_callable<T0>::value
                        and pythonic::types::is_callable<T1>::value,
                        pythonic::types::variant<T0,T1>
                        >::type
operator+(T0 , T1);

template <class T>
typename std::enable_if<pythonic::types::is_callable<T>::value,
                        T
                        >::type operator+(T , T);
...
struct __combined
{
    using type = typename __combined<T0,
                                     typename __combined<T1,
                                                         Types...
                                                         >::type
                                     >::type;
};
...
struct __combined<T0&, T1&>
{
    using type = typename std::add_lvalue_reference<typename __combined<T0, T1>::type>::type;
};
...
struct __combined<const T0&, const T1&>
{
    using type = typename std::add_lvalue_reference<typename std::add_const<typename __combined<T0, T1>::type>::type>::type;
};
...
class indexable_container
{
    public:
        ...
        using value_type = typename std::remove_cv< typename std::remove_reference<V>::type>::type;
    private:
        indexable_container();
};
...
{
    using type = indexable_container<typename __combined<K0,K1>::type, typename __combined<V0,V1>::type>;
};
