...
namespace pythonic {
    ...
    namespace types {
        ...
        template<class Arg, class... S>
            struct numpy_gexpr {
                ...
                using value_type = typename std::remove_reference<decltype(numpy_gexpr_helper<Arg, S...>::get(std::declval<numpy_gexpr>(), 0L))>::type;
                ...
                template<size_t J, class Slice>
                    typename std::enable_if<std::is_same<Slice, slice>::value
                        or std::is_same<Slice, contiguous_slice>::value, void>::type
    ...
    template<class Arg, class... S>
        ...
        {
            using type = types::numpy_gexpr<typename lazy<Arg>::type, typename lazy<S>::type...>;
        };

    template<class Arg, class... S>
        struct lazy<types::numpy_gexpr<Arg const &, S...>>
        {
            using type = types::numpy_gexpr<typename lazy<Arg>::type const &, typename lazy<S>::type...>;
        };
