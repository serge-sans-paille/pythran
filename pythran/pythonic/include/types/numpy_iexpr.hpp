    ...
    namespace types {
        ...
        template<class Arg> // Arg often is a reference, e.g. for something as simple as a[i]
            struct numpy_iexpr
            {
                ...
                using value_type = typename std::remove_reference<decltype(numpy_iexpr_helper<numpy_iexpr, value>::get(std::declval<numpy_iexpr>(), 0L))>::type;
                ...
                auto fast(long i) &&
                -> decltype(numpy_iexpr_helper<numpy_iexpr, value>::get(std::move(*this), i));
                ...
                template<class ...S>
                    numpy_gexpr<numpy_iexpr, slice, S...>
