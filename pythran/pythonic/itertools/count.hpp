...
namespace pythonic
{
  ...
  namespace itertools
  {
    ...
    {
      return count_iterator<T>(std::numeric_limits<T>::max(),
                               count_iterator<T>::step);
    }

    template <typename T0, typename T1>
    _count<decltype(std::declval<T0>() + std::declval<T1>())> count(T0 start,
                                                                    T1 step)
    {
      return _count<decltype(std::declval<T0>() + std::declval<T1>())>(start,
                                                                       step);
    }
