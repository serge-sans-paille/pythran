...
namespace pythonic
{
  ...
  namespace itertools
  {
    ...
    bool ifilter_iterator<ResultType, Operator, List0>::test_filter(
        std::false_type)
    {
      return op(*iter);
    }
    ...
    ifilter_iterator<ResultType, Operator, List0>::ifilter_iterator(
        ...
        : op(_op), iter(const_cast<sequence_type &>(_seq).begin()),
    ...
    ifilter_iterator<ResultType, Operator, List0>::ifilter_iterator(
        ...
        : op(_op), iter(const_cast<sequence_type &>(_seq).end()),
          iter_end(const_cast<sequence_type &>(_seq).end())
    {
    }
    ...
    template <typename ResultType, typename Operator, typename List0>
    ifilter_iterator<ResultType, Operator, List0> &
        ifilter_iterator<ResultType, Operator, List0>::
        operator++()
    {
      next_value();
    ...
    template <typename ResultType, typename Operator, typename List0>
    bool ifilter_iterator<ResultType, Operator, List0>::
    ...
    template <typename ResultType, typename Operator, typename List0>
    int ifilter_iterator<ResultType, Operator, List0>::
    operator-(ifilter_iterator const &other) const
    {
      return (iter - other.iter);
    }
    ...
    _ifilter<ResultType, Operator, List0>::_ifilter(Operator _op, List0 _seq)
        : iterator(_op, _seq), seq(_seq), end_iter(npos(), _op, _seq)
    {
    }
    ...
    _ifilter<ResultType, Operator, List0>::begin() const
    {
      return *this;
    }
