...
namespace pythonic
{
  ...
  namespace itertools
  {
    ...
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<
        Op, L0, It...>::imap_iterator_data()
    {
    }
    ...
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<
        Op, L0, It...>::imap_iterator_data(Op _op, First &&_seq,
                                           Types &&... _iters)
        : iterator_ref(_seq), iter(iterator_ref.begin()),
    ...
    template <typename Op, typename L0, typename... It>
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<
        Op, L0, It...>::imap_iterator_data(npos, imap_iterator_data &beg)
        : iterator_ref(beg.iterator_ref), iter(beg.iterator_ref.end()),
    ...
    auto imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<
        Op, L0, It...>::next_value(Types &&... params) const
    ...
    template <typename ResultType, typename Operator, typename... Iters>
    template <typename Op, typename L0, typename... It>
    bool imap_iterator<ResultType, Operator,
                       Iters...>::imap_iterator_data<Op, L0, It...>::
    operator==(imap_iterator_data const &other) const
    {
      return !(*this != other);
    }
    ...
    operator!=(imap_iterator_data const &other) const
    {
      return (iter != other.iter) and
             (rec_iters != other.rec_iters); // stop as soon as one is different
    }

    template <typename ResultType, typename Operator, typename... Iters>
    template <typename Op, typename L0, typename... It>
    int imap_iterator<ResultType, Operator,
                      Iters...>::imap_iterator_data<Op, L0, It...>::
    operator-(imap_iterator_data const &other) const
    {
    ...
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<Op, L0,
                                                                      It...> &
        imap_iterator<ResultType, Operator,
                      Iters...>::imap_iterator_data<Op, L0, It...>::
        operator++()
    {
      ++iter;
    ...
    {
      imap_iterator<ResultType, Operator,
                    Iters...>::imap_iterator_data<Op, L0, It...> other(*this);
      return other += i;
    }
    ...
    imap_iterator<ResultType, Operator,
                  Iters...>::imap_iterator_data<Op, L0>::imap_iterator_data()
    {
    }
    ...
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<
        Op, L0>::imap_iterator_data(Op _op, First &&_seq)
        : op(_op), iterator_ref(_seq), iter(iterator_ref.begin())
    {
    }
    ...
    template <typename Op, typename L0>
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<
        Op, L0>::imap_iterator_data(npos, imap_iterator_data &beg)
        : op(beg.op), iterator_ref(beg.iterator_ref),
    ...
    template <typename ResultType, typename Operator, typename... Iters>
    template <typename Op, typename L0>
    bool imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<Op,
                                                                           L0>::
    operator==(imap_iterator_data const &other) const
    {
      return !(iter != other.iter);
    ...
    template <typename ResultType, typename Operator, typename... Iters>
    template <typename Op, typename L0>
    int imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<Op,
                                                                          L0>::
    operator-(imap_iterator_data const &other) const
    {
      return (iter - other.iter);
    ...
    template <typename Op, typename L0>
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<Op, L0> &
        imap_iterator<ResultType, Operator, Iters...>::imap_iterator_data<Op,
                                                                          L0>::
        operator++()
    {
      ++iter;
    ...
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator(
        Operator _op, Types &&... _iters)
        : it_data(_op, std::forward<Types>(_iters)...)
    {
    }

    // constructor for the end iterator. Note how it uses a reference to another
    // iterator to initialize itself
    // that way we are sure to always iterate on the same data
    template <typename ResultType, typename Operator, typename... Iters>
    template <class... Types>
    imap_iterator<ResultType, Operator, Iters...>::imap_iterator(
        npos, imap_iterator &beg)
    ...
    template <typename ResultType, typename Operator, typename... Iters>
    bool imap_iterator<ResultType, Operator, Iters...>::
    operator==(imap_iterator<ResultType, Operator, Iters...> const &other) const
    {
    ...
    template <typename ResultType, typename Operator, typename... Iters>
    int imap_iterator<ResultType, Operator, Iters...>::
    operator-(imap_iterator<ResultType, Operator, Iters...> const &other) const
    {
      return it_data - other.it_data;
    }
    ...
    template <typename ResultType, typename Operator, typename... Iters>
    template <class... Types>
    _imap<ResultType, Operator, Iters...>::_imap(Operator &&_op,
                                                 Types &&... _iters)
        : imap_iterator<ResultType, Operator, Iters...>(
              _op, std::forward<Types>(_iters)...),
          end_iter(npos(), *this) // NOTE: end_iter is initialised using npos to
                                  // distinguish it from begin,
    // and using a reference to begin (i.e. this) to avoid iterating over
    // different
    // objects.
    {
    }
    ...
    template <typename ResultType, typename Operator, typename... Iters>
    typename _imap<ResultType, Operator, Iters...>::iterator const &
    _imap<ResultType, Operator, Iters...>::end() const
    {
      return end_iter;
    }
