...
namespace pythonic {
    ...
    namespace numpy {
        ...
        template<class E>
            auto around(E const& a, long decimals=0)
            -> typename std::enable_if<std::is_integral<typename types::numpy_expr_to_ndarray<E>::type::dtype>::value,
               decltype(a / nt2::pow(typename types::numpy_expr_to_ndarray<E>::type::dtype(10),std::max(0L,-decimals)) * nt2::pow(typename types::numpy_expr_to_ndarray<E>::type::dtype(10), std::max(0L,-decimals)))
                 >::type
            {
                return a / nt2::pow(typename types::numpy_expr_to_ndarray<E>::type::dtype(10),std::max(0L,-decimals)) * nt2::pow(typename types::numpy_expr_to_ndarray<E>::type::dtype(10), std::max(0L,-decimals));
            }
