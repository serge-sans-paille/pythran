...
namespace pythonic {
    ...
    namespace numpy {
        template<class T, size_t N>
            types::ndarray<T,1> delete_(types::ndarray<T,N> const& a, long index, types::none_type axis)
            {
                types::ndarray<T,1> out(types::make_tuple(long(a.flat_size())-1), __builtin__::None);
                long n = a.flat_size();
        ...
        template<class T, size_t N, class I>
            ...
            delete_(types::ndarray<T,N> const& in, I const& indices, types::none_type axis)
            {
