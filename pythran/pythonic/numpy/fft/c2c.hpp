...
namespace numpy
{
  namespace fft
  {
    ...
    types::ndarray<typename std::enable_if<std::is_integral<T>::value, double>::type,
    ...
    template<typename T> T norm_fct(Inorm inorm, size_t N)
    {
        switch (inorm)
        {
            case Inorm::ortho : return T(1/sqrt(ldbl_t(N)));
            case Inorm::backward : return T(1/ldbl_t(N));
            case Inorm::forward : return T(1);
        }
    }

    template<typename T> T norm_fct(Inorm inorm, const shape_t &shape,
    const shape_t &axes, size_t fct=1, int delta=0)
    {
    if (inorm==Inorm::forward) return T(1);
    size_t N(1);
    for (auto a: axes)
    ...
    {
      types::ndarray<T, types::array<long, std::tuple_size<pS>::value>> extended_array;
      auto tmp_shape = sutils::getshape(in_array);
      tmp_shape[axis] = n;
      auto tmp_array = zeros(tmp_shape, types::dtype_t<T>());
      types::list<types::ndarray<T, types::array<long, std::tuple_size<pS>::value>>> bi(0);
