...
namespace pythonic {
    ...
    namespace numpy {
        ...
        template<class T, size_t N, class I, class F>
            ...
            insert(types::ndarray<T,N> in, I const& indices, F const& data, types::none_type axis=__builtin__::None)
            {
                types::ndarray<T,1> out(types::make_tuple(long(in.flat_size()+std::min(indices.flat_size(), data.flat_size()))), __builtin__::None);
