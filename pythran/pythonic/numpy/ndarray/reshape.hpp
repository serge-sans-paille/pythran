...
namespace numpy
{
  namespace ndarray
  {
    ...
    std::enable_if_t<!std::is_integral_v<NpS>, types::ndarray<T, NpS>>
    reshape(types::ndarray<T, pS> const &expr, NpS const &new_shape)
    {
      long where =
          sutils::sfind(new_shape, -1, std::integral_constant<size_t, std::tuple_size_v<NpS>>(),
      ...
      if (where >= 0) {
        ...
        misc::set(auto_shape, where, expr.flat_size() / -sutils::sprod(new_shape),
                  std::make_index_sequence<std::tuple_size_v<NpS>>());
    ...
    template <class T, class pS, class NpS>
    std::enable_if_t<std::is_integral_v<NpS>, types::ndarray<T, types::pshape<long>>>
    reshape(types::ndarray<T, pS> const &expr, NpS const &new_shape)
    {
      auto n = expr.flat_size();
      if (new_shape <= -1) {
