...
namespace numpy
{
  ...
  template <class E>
  auto nonzero(E const &expr) -> types::array<types::ndarray<long, types::array<long, 1>>, E::value>
  {
    constexpr long N = E::value;
    typedef types::array<types::ndarray<long, types::array<long, 1>>, E::value> out_type;
    ...
    out_type out;
    for (size_t i = 0; i < N; ++i)
      out[i] = types::ndarray<long, types::array<long, 1>>(std::move(out_buffers[i]), shape);
