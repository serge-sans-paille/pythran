    ...
    namespace types {
        ...
        template<class Op, class... Args>
            numpy_expr<Op, Args...>::numpy_expr(Args const &...args) :
                args(args...),
                _shape(std::get<utils::max_element<Args::value...>::index>(this->args).shape())
            {}

        template<class Op, class... Args>
            typename numpy_expr<Op, Args...>::iterator numpy_expr<Op, Args...>::begin() const
            {
                return {*this, 0};
            }

        template<class Op, class... Args>
            typename numpy_expr<Op, Args...>::iterator numpy_expr<Op, Args...>::end() const
            {
        ...
        template<class Op, class... Args>
            ...
            -> decltype(this->_fast(i, typename utils::gens<sizeof...(Args)>::type{}))
            {
        ...
        template<class Op, class... Args>
            array<long, numpy_expr<Op, Args...>::value> const& numpy_expr<Op, Args...>::shape() const
            {
                return _shape;
            }
        ...
        template<class I> // template to prevent automatic instantiation when the type is not vectorizable
            ...
            -> decltype(this->_load(i, typename utils::gens<sizeof...(Args)>::type{}))
            {
        ...
        auto
        numpy_expr<Op, Args...>::_get(utils::seq<I...>, S const&... s) const
        ...
        template<class Op, class... Args>
        template<class F>
            typename std::enable_if<is_numexpr_arg<F>::value, numpy_fexpr<numpy_expr<Op, Args...>, F>>::type
