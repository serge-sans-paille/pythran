...
namespace pythonic {
    ...
    namespace types {
        ...
        template<class Arg, class F>
            array<long, numpy_fexpr<Arg, F>::value> const& numpy_fexpr<Arg, F>::shape() const
            {
                return _shape;
            }
        ...
        template<class FIter, class O>
            void numpy_fexpr<Arg, F>::_copy_mask(FIter fiter, FIter fend, O& out, long &index, utils::int_<1>)
            {
        ...
        template<class FIter, class O, size_t N>
            void numpy_fexpr<Arg, F>::_copy_mask(FIter fiter, FIter fend, O& out, long & index, utils::int_<N>)
            {
        ...
        template<class E>
            ...
            numpy_fexpr<Arg, F>::operator=(E const& expr)
            {
        ...
        template<class Arg, class F>
        template<class E>
            typename std::enable_if<not is_iterable<E>::value, numpy_fexpr<Arg, F>&>::type
            numpy_fexpr<Arg, F>::operator=(E const& expr)
            {
        ...
        template<class Arg, class F>
            typename numpy_fexpr<Arg, F>::const_iterator numpy_fexpr<Arg, F>::begin() const
            {
                return {*this, 0};
            }
        ...
        template<class Arg, class F>
        template<class E>
            typename std::enable_if<is_numexpr_arg<E>::value, numpy_fexpr<numpy_fexpr<Arg, F>, E>>::type
        ...
        template<class E>
            typename std::enable_if<is_numexpr_arg<E>::value, numpy_fexpr<numpy_fexpr<Arg, F>, E>>::type
            numpy_fexpr<Arg, F>::operator[](E const& expr) const
            {
                return fast(expr);
            }
