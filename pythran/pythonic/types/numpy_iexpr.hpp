...
namespace pythonic {
    ...
    namespace types {
        ...
        template<class Arg>
            numpy_iexpr<Arg>::numpy_iexpr(typename std::remove_reference<Arg>::type &&arg, long index) :
                arg(std::move(arg)),
                buffer(arg.buffer)
            {
                buffer += buffer_offset(index, utils::int_<value>());
        ...
        template<class Arg>
            ...
            {
                return make_const_nditerator<is_strided or value != 1>()(*this, _shape[0]);
            }
        ...
        template<class Arg>
            auto numpy_iexpr<Arg>::fast(long i) &&
            -> decltype(numpy_iexpr_helper<numpy_iexpr<Arg>, value>::get(std::move(*this), i))
            {
        ...
        template<class Arg>
            auto numpy_iexpr<Arg>::operator()(long i) &&-> decltype(std::move(*this)[i])
            {
                return std::move(*this)[i];
            }

        template<class Arg>
            numpy_gexpr<numpy_iexpr<Arg>, slice>
            numpy_iexpr<Arg>::operator()(slice const& s0) const
            {
        ...
        template<class Arg>
            numpy_gexpr<numpy_iexpr<Arg>, contiguous_slice>
            numpy_iexpr<Arg>::operator()(contiguous_slice const& s0) const
            {
                return make_gexpr(*this, s0);
            }
        ...
        template<class Arg>
        template<class ...S>
            numpy_gexpr<numpy_iexpr<Arg>, slice, S...>
        ...
        template<class Arg>
        template<class ...S>
            numpy_gexpr<numpy_iexpr<Arg>, contiguous_slice, S...>
            numpy_iexpr<Arg>::operator()(contiguous_slice const& s0, S const&... s) const
            {
                return make_gexpr(*this, s0, s...);
            }
        ...
        template<class Arg>
        template<class F>
            typename std::enable_if<is_numexpr_arg<F>::value, numpy_fexpr<numpy_iexpr<Arg>, F>>::type
