...
namespace pythonic {
    ...
    namespace types {
        ...
        contiguous_normalized_slice::contiguous_normalized_slice(long lower, long upper) :
            lower(lower),
            upper(upper)
        {}
        ...
        contiguous_slice::contiguous_slice(none<long> lower, none<long> upper) :
            lower(lower.is_none?0:(long)lower),
            upper(upper)
        {}
        ...
        contiguous_slice contiguous_slice::operator*(contiguous_slice const& other) const
        {
            long new_lower;
            if(other.lower < 0)
