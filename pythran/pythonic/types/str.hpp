...
namespace pythonic {
    ...
    namespace types {
        ...
        const_sliced_str_iterator::const_sliced_str_iterator(char const *data, long step) :
            data(data),
            step(step)
        {}
        ...
        bool const_sliced_str_iterator::operator<(const_sliced_str_iterator const& other) const
        {
        ...
        long const_sliced_str_iterator::operator-(const_sliced_str_iterator const & other) const
        {
        ...
        template<class S>
            typename S::normalized_type const &
            sliced_str<S>::get_slice() const
            {
                return slicing;
            }
        ...
        template<class S>
            ...
            // size
        template<class S>
            typename sliced_str<S>::size_type sliced_str<S>::size() const
            {
                return slicing.size();
            }
        ...
        template<class S>
            sliced_str<contiguous_slice>
            sliced_str<S>::operator[](contiguous_slice const& s) const
            {
...
namespace std {
    ...
    size_t hash<pythonic::types::str>::operator()(const pythonic::types::str & x) const
    {
...
namespace pythonic {
    ...
    void python_to_pythran< types::str >::construct(PyObject* obj_ptr, boost::python::converter::rvalue_from_python_stage1_data* data)
    {
    ...
    PyObject* custom_pythran_sliced_str_to_str::convert(const types::sliced_str<types::contiguous_slice>& v)
    {
        return custom_pythran_string_to_str().convert(v);
    }

    PyObject* custom_pythran_sliced_str_to_str::convert(const types::sliced_str<types::slice>& v)
    {
        return custom_pythran_string_to_str().convert(v);
    }
