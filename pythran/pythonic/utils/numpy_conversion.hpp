...
template<class E, class... Types>                                               \
    ...
    -> typename std::enable_if<not types::is_ndarray<E>::value and types::is_array<E>::value,\
                               decltype(fname(typename types::numpy_expr_to_ndarray<E>::type(expr), std::forward<Types>(others)...))\
                               >::type;
