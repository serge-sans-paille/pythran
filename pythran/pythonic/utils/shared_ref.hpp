...
namespace pythonic {
    ...
    namespace utils {
        ...
        template <class T>
            shared_ref<T>::shared_ref(no_memory const&) noexcept :
                mem(nullptr)
                {}

        template <class T>
            shared_ref<T>::shared_ref(no_memory &&) noexcept :
                mem(nullptr)
                {}
        ...
        template <class T>
            shared_ref<T>::shared_ref(shared_ref<T>&& p) noexcept :
                mem(p.mem)
                {
                    p.mem=nullptr;
                }
        ...
        template <class T>
            shared_ref<T>::shared_ref(shared_ref<T> const& p) noexcept :
                mem(p.mem)
                {
                    if(mem)
                        acquire();
