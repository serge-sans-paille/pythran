...
import ast
import cxxtypes
from intrinsic import ConstFunctionIntr, FunctionIntr, ConstMethodIntr, MethodIntr, AttributeIntr, ScalarIntr
...
modules = {
        ...
        "__set__" : {
                ...
                "union_" : ConstMethodIntr([ lambda self, node: [ self.combine(node.args[0], node_args_k, unary_op=lambda f: cxxtypes.SetType(cxxtypes.ContentType(f)), register=True) for node_args_k in node.args[1:] ]]),
                "intersection" : ConstMethodIntr([ lambda self, node: [ self.combine(node.args[0], node_args_k, unary_op=lambda f: cxxtypes.SetType(cxxtypes.ContentType(f)), register=True) for node_args_k in node.args[1:] ]]),
                ...
                "difference" : ConstMethodIntr([ lambda self, node: [ self.combine(node.args[0], node_args_k, unary_op=lambda f: cxxtypes.SetType(cxxtypes.ContentType(f)), register=True) for node_args_k in node.args[1:] ]]),
                ...
                "symmetric_difference" : ConstMethodIntr([ lambda self, node: [ self.combine(node.args[0], node_args_k, unary_op=lambda f: cxxtypes.SetType(cxxtypes.ContentType(f)), register=True) for node_args_k in node.args[1:] ]]),
