...
""", [True,False,True,True], ifiltern_=[[bool]])
    ...
    def test_next_imap(self):
        self.run_test("def next_imap(n): from itertools import imap ; x = imap(abs,n) ; next(x) ; return map(None, x)", list(range(-5,5)), next_imap=[[int]])

    @unittest.skipIf(sys.version_info.major == 3, "not supported in pythran3")
    def test_next_imap_none(self):
        self.run_test("def next_imap_none(n): from itertools import imap ; x = imap(None,n) ; next(x) ; return map(None, x)", list(range(-5,5)), next_imap_none=[[int]])

    @unittest.skipIf(sys.version_info.major == 3, "not supported in pythran3")
    def test_next_ifilter(self):
        self.run_test("def next_ifilter(n): from itertools import ifilter ; x = ifilter(abs,n) ; next(x) ; return map(None, x)", list(range(-5,5)), next_ifilter=[[int]])

    @unittest.skipIf(sys.version_info.major == 3, "not supported in pythran3")
    def test_next_ifilter_none(self):
        self.run_test("def next_ifilter_none(n): from itertools import ifilter ; x = ifilter(None,n) ; next(x) ; return map(None, x)", list(range(-5,5)), next_ifilter_none=[[int]])
    ...
    def test_next_izip(self):
        self.run_test("def next_izip(n): from itertools import izip ; x = izip(n,n) ; next(x) ; return map(None, x)", list(range(-5,5)), next_izip=[[int]])
