run_tests() {
    ...
    then
        if [ "$PYTHON_VERSION" = 3 ]
        then
            PYTHONPATH=$PWD:$PYTHONPATH pytest pythran/tests/test_xdoc.py pythran/tests/notebooks docs/examples --nbval
        else
            PYTHONPATH=$PWD:$PYTHONPATH pytest pythran/tests/test_xdoc.py
        fi
        exit
    fi
    printf "[compiler]\nCXX=$CXX\nCC=$CC\ncflags=-std=c++11 $CXXFLAGS -w -O0 -fvisibility=hidden -fno-wrapv\nldflags=$CXXFLAGS -fopenmp -fvisibility=hidden -Wl,-strip-all\n" > ~/.pythranrc
    OMP_NUM_THREADS=3 PYTHONPATH=$PWD:$PYTHONPATH pytest -v $TESTCASE
}
