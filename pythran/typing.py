...
class GlobalsUpdatedByFunctions(ReturnTypeDependencies):
    def run(self, node, ctx):
        super(GlobalsUpdatedByFunctions, self).run(node, ctx)
...
class Callees(ModuleAnalysis):
    ...
    list of arguments with which they are called
    ...
    def visit_Call(self, node):
        ...
        gb_vals = self.global_declarations.values()
        res = {y for y in res
               if y in gb_vals and isinstance(y, ast.FunctionDef)}
...
class AcyclicCallees(ModuleAnalysis):
    ...
    arbitrarily is here based on the order the ast nodes describing the
    functions have in a dictionary"""
    def __init__(self):
        self.result = {}
...
class Types(ModuleAnalysis):
    ...
    def __init__(self):
        ...
        ModuleAnalysis.__init__(self, StrictAliases, LazynessAnalysis,
                                DeclaredGlobals, GlobalsUpdatedByFunctions,
            ...
            if register:  # this comes from an assignment,
                ...
                if depth > 0 and not is_global:
                    ...
                    if node_id in self.current_global_combiner:
                        ...
                        #The globals_updated_by_functions analysis decided to
    ...
    def visit_FunctionDef(self, node):
        ...
        gbupdated = self.globals_updated_by_functions
