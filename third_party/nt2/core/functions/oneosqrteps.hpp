...
//==============================================================================
#ifndef NT2_CORE_FUNCTIONS_ONEOSQRTEPS_HPP_INCLUDED
...
namespace nt2
{
  ...
  /*!
    ...
    - For any Integer @c n and any type @c T, the following code:
      @code
      auto x = oneosqrteps(n, as_<T>());
      @endcode
      generates an expression that evaluates as a @size2d{n,n} table filled with
      type @c T oneosqrtepss.

    - For any Integer @c sz1,...,szN and any type @c T, the following code:
      @code
      auto x = oneosqrteps(sz1,...,szn, as_<T>());
      @endcode
      generates an expression that evaluates as a @sizes{sz1,szn} table filled
      with type @c T oneosqrtepss.

    - For any Expression @c dims evaluating as a row vector of @c N elements
      and any type @c T, the following code:
      @code
      auto x = oneosqrteps(dims, as_<T>());
      @endcode
      generates an expression that evaluates as a @sizes{dims(1),dims(N)}
      table filled with type @c T oneosqrtepss.
    ...
    @return An Expression evaluating as an array of a given type and dimensions
            filled with the @c Oneosqrteps constant.

    @sa Oneosqrteps
  **/
  template<typename... Args, typename ClassName>
  details::unspecified oneosqrteps(Args const&... dims, ClassName const& classname);

  /// @overload
  template<typename... Args> details::unspecified oneosqrteps(Args const&... dims);

  /// @overload
  template<typename ClassName> ClassName::type oneosqrteps(ClassName const& classname);
  ...
  #define M0(z,n,t)                                                            \
  NT2_FUNCTION_IMPLEMENTATION(nt2::tag::Oneosqrteps,oneosqrteps, n)                            \
  /**/
...
namespace nt2 { namespace ext
{
  ...
  struct  value_type<tag::Oneosqrteps,Domain,N,Expr>
        : meta::generative_value<Expr>
  {};

  /// INTERNAL ONLY
  template<typename Domain, typename Expr, int N>
  struct  size_of<tag::Oneosqrteps,Domain,N,Expr>
